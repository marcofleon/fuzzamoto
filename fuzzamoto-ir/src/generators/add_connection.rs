...
/// `AddConnectionAndHandshakeGenerator` generates programs that create a new connection and
/// perform a handshake
#[derive(Default)]
pub struct AddConnectionAndHandshakeGenerator;

impl<R: RngCore> Generator<R> for AddConnectionAndHandshakeGenerator {
    fn generate(&self, builder: &mut ProgramBuilder, rng: &mut R) -> GeneratorResult {
        ...
        for _ in 0..num_connections {
            ...
            let conn_type = if rng.gen_bool(0.5) { "inbound" } else { "outbound" };
            let conn_type_var = builder
                .append(Instruction {
                    inputs: vec![],
                    operation: Operation::LoadConnectionType(conn_type.to_string()),
            ...
            builder
                .append(Instruction {
                    ...
                    operation: Operation::AddConnectionAndHandshake,
                })
                .expect("Inserting AddConnectionAndHandshake should always succeed");
        }
        Ok(())
    }

    fn name(&self) -> &'static str {
        "AddConnectionAndHandshakeGenerator"
    }
}
